...
from cobra.experiments.pdp.solvers.basic_genetic import Genetic_PDP_Solver
...
if __name__ == "__main__":
    ...
    solver = Genetic_PDP_Solver(env=env, fitness_fn=fitness)
    updated = True
    ...
    while True:
        if env.get_current_timestep() % 300 == 0:
            ...
            if env.are_new_request_createable():
                env.generate_requests(limited_by_locations=True, max_new_requests=4)
            data = env.get_data()
            pairs = data["pickups_deliveries"]
            print(f"Pairs: {pairs}")
            routes = solver.solve(data=data, population_size=50, num_iterations=50)
