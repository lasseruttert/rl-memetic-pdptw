...
# this is the operator M4 from the paper "Large Neighborhood Search with adaptive guided ejection search for the pickup and delivery problem with time windows"

class CLSM4Operator(BaseOperator):
    """Un-assign a PD pair (pd1) from a route (r1), un-assign a PD pair (pd2) from a route (r2) and then try and insert pd1 into route r2 and pd2 into a third route r3"""
    def __init__(self, insertion_heuristic: str = 'greedy'):
        ...
        self.name = f'CLS-M4-{insertion_heuristic}'
    ...
    def apply(self, problem: PDPTWProblem, solution: PDPTWSolution) -> PDPTWSolution:
        ...
        # insert request 2 into a different route
        self.insertion_heuristic.not_allowed_vehicle_idxs = [route_with_request_1, route_with_request_2]
