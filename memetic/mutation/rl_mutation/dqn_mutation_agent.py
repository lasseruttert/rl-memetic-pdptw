"""DQN agent for learning mutation operator selection policy."""

import torch
import torch.nn.functional as F
import numpy as np
from typing import Optional, Tuple
from memetic.mutation.rl_mutation.dqn_mutation_network import DQNMutationNetwork


class DQNMutationAgent:
    """DQN agent for learning mutation operator selection policy."""
    ...
    def __init__(
        ...
        self.q_network = DQNMutationNetwork(
            state_dim=state_dim,
            action_dim=action_dim,
            hidden_dims=hidden_dims
        ).to(self.device)

        self.target_network = DQNMutationNetwork(
            state_dim=state_dim,
            action_dim=action_dim,
            hidden_dims=hidden_dims
        ).to(self.device)
