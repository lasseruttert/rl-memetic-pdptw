...
RESULTS_OUTPUT_FILE = "results/memetic_component_results_100_avg.json"
SUMMARY_CSV_FILE = "results/memetic_component_summary_100_avg.csv"
...
def create_local_search_instances(operator_dict):
    ...
    ls_06_rl_set1_one_shot = RLLocalSearch.load_from_checkpoint("models/rl_local_search_dqn_100_greedy_binary_100_set1_final.pt")
    ...
    ls_09_rl_set2_ls_init = RLLocalSearch.load_from_checkpoint("models/rl_local_search_dqn_100_greedy_binary_100_set2_ls_init_final.pt")
    local_searches.append(ls_09_rl_set2_ls_init)

    ls_10_rl_set2_shorter = RLLocalSearch.load_from_checkpoint("models/rl_local_search_dqn_100_greedy_binary_100_set2_shorter_final.pt")
...
def run_experiment():
    """Run memetic component-wise performance experiment with averaged results.

    Tests different combinations of memetic algorithm components across all instances.
    Results are averaged across multiple runs instead of taking the best run.
    Results are saved incrementally to allow for interrupted runs.
    """
    print("=" * 80)
    print("MEMETIC COMPONENT-WISE PERFORMANCE EXPERIMENT (AVERAGED)")
    print("=" * 80)
    print(f"Problem sizes: {PROBLEM_SIZES}")
    print(f"Population size: {POPULATION_SIZE}")
    print(f"Max time per instance: {MAX_TIME_SECONDS} seconds")
    print(f"Number of runs per instance: {NUM_RUNS}")
    ...
    for combo_idx, combination in enumerate(COMBINATIONS):
        ...
        for inst_idx, instance in enumerate(all_instances):
            ...
            for i in range(NUM_RUNS):
                # Set different seed for each run to ensure independent samples
                seed = inst_idx * NUM_RUNS + i
                random.seed(seed)
                np.random.seed(seed)
                ...
                all_run_results.append(instance_results)

            # Average the results (with standard deviations)
            avg_results = {
                'instance_name': instance_name,
                'bks_fitness': bks_fitness,
                'bks_num_vehicles': bks_num_vehicles,
                'bks_total_distance': bks_total_distance,
                # Fitness
                'avg_fitness': float(np.mean([r['best_fitness'] for r in all_run_results])),
                'std_fitness': float(np.std([r['best_fitness'] for r in all_run_results])),
                # Vehicles
                'avg_num_vehicles': float(np.mean([r['best_num_vehicles'] for r in all_run_results])),
                'std_num_vehicles': float(np.std([r['best_num_vehicles'] for r in all_run_results])),
                # Distance
                'avg_total_distance': float(np.mean([r['best_total_distance'] for r in all_run_results])),
                'std_total_distance': float(np.std([r['best_total_distance'] for r in all_run_results])),
                # Feasibility (rate, no std needed - it's a proportion)
                'feasibility_rate': float(np.mean([r['is_feasible'] for r in all_run_results])),
                # Time
                'avg_total_time': float(np.mean([r['total_time'] for r in all_run_results])),
                'std_total_time': float(np.std([r['total_time'] for r in all_run_results])),
                # Time to best
                'avg_time_to_best': float(np.mean([r['final_evaluation'].get('time_to_final_fitness', 0) for r in all_run_results])),
                'std_time_to_best': float(np.std([r['final_evaluation'].get('time_to_final_fitness', 0) for r in all_run_results])),
                # Num improvements
                'avg_num_improvements': float(np.mean([r['final_evaluation'].get('num_improvements', 0) for r in all_run_results])),
                'std_num_improvements': float(np.std([r['final_evaluation'].get('num_improvements', 0) for r in all_run_results])),
                # Longest stagnation
                'avg_longest_stagnation': float(np.mean([r['final_evaluation'].get('longest_stagnation', 0) for r in all_run_results])),
                'std_longest_stagnation': float(np.std([r['final_evaluation'].get('longest_stagnation', 0) for r in all_run_results])),
            }

            combination_results['instances'][instance_name] = avg_results

            print(f"    Avg fitness: {avg_results['avg_fitness']:.2f} (±{avg_results['std_fitness']:.2f}), "
                  f"Avg vehicles: {avg_results['avg_num_vehicles']:.2f} (±{avg_results['std_num_vehicles']:.2f}), "
                  f"Feasibility rate: {avg_results['feasibility_rate']:.2f}, "
                  f"Avg time: {avg_results['avg_total_time']:.2f}s")
        ...
        combo_file = f"{RESULTS_BASE_DIR}/combo_{combo_idx}_{combo_name}_{PROBLEM_SIZES[0]}_avg.json"
    ...
    for combo_id, combo_data in results.items():
        for instance_name, instance_data in combo_data['instances'].items():
            row = {
                ...
                'Avg_Fitness': instance_data['avg_fitness'],
                'Std_Fitness': instance_data['std_fitness'],
                'Avg_Vehicles': instance_data['avg_num_vehicles'],
                'Std_Vehicles': instance_data['std_num_vehicles'],
                'Avg_Distance': instance_data['avg_total_distance'],
                'Std_Distance': instance_data['std_total_distance'],
                'Feasibility_Rate': instance_data['feasibility_rate'],
                'Avg_Total_Time': instance_data['avg_total_time'],
                'Std_Total_Time': instance_data['std_total_time'],
                'Avg_Time_To_Best': instance_data['avg_time_to_best'],
                'Std_Time_To_Best': instance_data['std_time_to_best'],
                'Avg_Num_Improvements': instance_data['avg_num_improvements'],
                'Std_Num_Improvements': instance_data['std_num_improvements'],
                'Avg_Longest_Stagnation': instance_data['avg_longest_stagnation'],
                'Std_Longest_Stagnation': instance_data['std_longest_stagnation'],
            }
            csv_rows.append(row)
