"""
RL Mutation Performance Experiment

Tests trained RL mutation across all benchmark instances with both random
and greedy initial solutions. Tracks comprehensive metrics including population
diversity and saves detailed results.
"""
...
from memetic.mutation.rl_mutation import RLMutation
...
# Population size (required for mutation)
POPULATION_SIZE = 10

# Path to trained RL mutation model
MODEL_PATH = "models/rl_mutation_final.pt"

# Results directory
RESULTS_DIR = "results/rl_mutation_performance"
...
def generate_population(instance, population_size: int, random_generator, greedy_generator):
    """Generate a mixed population for mutation experiments.

    Args:
        instance: Problem instance
        population_size: Number of solutions in population
        random_generator: Random solution generator
        greedy_generator: Greedy solution generator
    ...
    num_greedy = population_size // 2
    num_random = population_size - num_greedy

    # Generate greedy solutions
    population.extend(greedy_generator.generate(instance, num_greedy))

    # Generate random solutions
    population.extend(random_generator.generate(instance, num_random))
    return population
...
def compute_averaged_metrics(run_results: List[Dict]) -> Dict:
    ...
    tw_violations = [r['avg_time_window_violations'] for r in run_results]
    cap_violations = [r['avg_capacity_violations'] for r in run_results]

    # Route metrics
    num_routes = [r['avg_num_routes'] for r in run_results]
    avg_route_distances = [r['avg_route_distance'] for r in run_results]

    # Population diversity metrics (NEW for mutation)
    avg_distances_to_pop = [r['avg_distance_to_population'] for r in run_results]
    min_distances_to_pop = [r['min_distance_to_population'] for r in run_results]
    ...
    averaged = {
        ...
        'mean_time_window_violations': float(np.mean(tw_violations)),
        'std_time_window_violations': float(np.std(tw_violations)),
        'mean_capacity_violations': float(np.mean(cap_violations)),
        'std_capacity_violations': float(np.std(cap_violations)),
        ...
        'mean_avg_distance_to_pop': float(np.mean(avg_distances_to_pop)),
        'std_avg_distance_to_pop': float(np.std(avg_distances_to_pop)),
        'mean_min_distance_to_pop': float(np.mean(min_distances_to_pop)),
        'std_min_distance_to_pop': float(np.std(min_distances_to_pop)),
        # Operator statistics
...
def analyze_run_history(run_history: Dict) -> Dict:
    ...
    # Constraint violations (average across iterations)
    tw_violations = [run_history[i]['time_window_violations'] for i in iterations]
    cap_violations = [run_history[i]['capacity_violations'] for i in iterations]
    ...
    # Population diversity metrics (NEW for mutation)
    avg_distances_to_pop = [run_history[i]['avg_distance_to_pop'] for i in iterations]
    min_distances_to_pop = [run_history[i]['min_distance_to_pop'] for i in iterations]
    ...
    return {
        ...
        'avg_time_window_violations': float(np.mean(tw_violations)) if tw_violations else 0.0,
        'avg_capacity_violations': float(np.mean(cap_violations)) if cap_violations else 0.0,
        ...
        'avg_distance_to_population': float(np.mean(avg_distances_to_pop)) if avg_distances_to_pop else 0.0,
        'min_distance_to_population': float(np.mean(min_distances_to_pop)) if min_distances_to_pop else 0.0,
...
def run_experiment():
    """Run RL mutation performance experiment.

    Tests trained RL mutation on all instances with random and greedy initialization.
    Saves comprehensive tracking data and statistics.
    """
    print("=" * 80)
    print("RL MUTATION PERFORMANCE EXPERIMENT")
    print("=" * 80)
    ...
    rl_mutation = RLMutation.load_from_checkpoint(MODEL_PATH, verbose=True)

    # Enable tracking
    rl_mutation.tracking = True
    ...
    for inst_idx, instance in enumerate(all_instances):
        ...
        # Generate population for this instance (required for mutation)
        print("  Generating population...")
        population = generate_population(instance, POPULATION_SIZE, random_generator, greedy_generator)
        ...
        for init_type, generator in [('random', random_generator), ('greedy', greedy_generator)]:
            ...
            for run_idx in range(NUM_RUNS):
                ...
                # Apply RL mutation with tracking
                result = rl_mutation.mutate(
                    problem=instance,
                    solution=initial_solution,
                    population=population
                )

                # Unpack result (tracking enabled returns 2 values for mutation)
                best_solution, run_history = result
            ...
            best_run_filename = f"rl_mutation_instance_{instance_name}_{init_type}_init_best_run_full_history.json"
        ...
        averaged_filename = f"rl_mutation_instance_{instance_name}_averaged_across_runs.json"
    ...
    summary_data = {
        'experiment': 'RL Mutation Performance',
    ...
    summary_filename = "rl_mutation_all_instances_summary.json"
    ...
    for instance_name, instance_data in all_results.items():
        for init_type in ['random', 'greedy']:
            ...
            row = {
                ...
                'Mean_TW_Violations': metrics['mean_time_window_violations'],
                'Mean_Cap_Violations': metrics['mean_capacity_violations'],
                'Mean_Num_Routes': metrics['mean_num_routes'],
                'Mean_Avg_Distance_To_Pop': metrics['mean_avg_distance_to_pop'],
                'Mean_Min_Distance_To_Pop': metrics['mean_min_distance_to_pop']
            }
            csv_rows.append(row)

    # Write CSV
    csv_filename = Path(RESULTS_DIR) / "rl_mutation_all_instances_averaged_metrics.csv"
    if csv_rows:
